...
export class TTMLParser {
  constructor(private dataManager: TTMLDataManager) {}

  public parseDocument(document: TextDocument): TTMLDocument {
    return this.parse(document.getText(), this.dataManager.getVoidElements(document.languageId));
  }

  public parse(text: string, voidElements: string[]): TTMLDocument {
    const scanner = createScanner(text, undefined, undefined, true);

    const ttmlDocument = new Node(0, text.length, [], void 0);
    let curr = ttmlDocument;
    ...
    return {
      roots: ttmlDocument.children,
      findNodeBefore: ttmlDocument.findNodeBefore.bind(ttmlDocument),
      findNodeAt: ttmlDocument.findNodeAt.bind(ttmlDocument)
    };
  }
}